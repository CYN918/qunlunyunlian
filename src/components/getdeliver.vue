<template>
    <div class="dynamic">
        <mt-header title="确定发布">
            <router-link to="/myaccepttaskslist" slot="left">
                <mt-button icon="back">返回</mt-button>
            </router-link>
        </mt-header>

        <mt-field label="动态内容" placeholder="动态内容" type="textarea" rows="4" v-model="introduction"></mt-field>
        <listFile @getFiles="updateImg" sendsrt="动态附件"></listFile>
        <div>
	  ...
	  name: 'dynamic',
		...
		methods: {
            ...
            refergo () {
                ...
                this.subumting = true
                axios.post(this.baseCont.PHP_API + '/user/getdeliver', {
                ...
                .then(response => {
                    ...
                    if(response.data.code == 200){
                        ...
                        this.$router.push({path: '/myaccepttaskslist'});
                    }else{
                        Toast({
                            message: response.data.message,
                            duration: 2000
